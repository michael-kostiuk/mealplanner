...
def test_ingredients(db_session):
    ingredients = [
        models.Ingredient(id=1, name="сіль", category="Spices", base_unit="g", calories=0, protein=0, carbs=0, fats=0),
        models.Ingredient(id=2, name="salt", category="Spices", base_unit="g", calories=0, protein=0, carbs=0, fats=0),
        models.Ingredient(id=3, name="перець", category="Spices", base_unit="g", calories=0, protein=0, carbs=0, fats=0),
        models.Ingredient(id=4, name="black pepper", category="Spices", base_unit="g", calories=0, protein=0, carbs=0, fats=0),
        models.Ingredient(id=5, name="цибуля", category="Vegetables", base_unit="g", calories=0, protein=0, carbs=0, fats=0),
        models.Ingredient(id=6, name="onion", category="Vegetables", base_unit="g", calories=0, protein=0, carbs=0, fats=0),
    ]
...
def test_recipes(db_session, test_ingredients):
    recipes = [
        models.Recipe(
            id=1, name="Recipe 1", servings=4, prep_time=10, cook_time=20,
            instructions="Test instructions", category="Test", calories=100,
            protein=10, carbs=20, fats=5, breakfast_weight=0.5, lunch_weight=0.5, dinner_weight=0.5
        ),
        models.Recipe(
            id=2, name="Recipe 2", servings=2, prep_time=15, cook_time=30,
            instructions="Test instructions 2", category="Test", calories=150,
            protein=15, carbs=25, fats=10, breakfast_weight=0.3, lunch_weight=0.7, dinner_weight=0.3
        ),
    ]
...
def test_shopping_lists(db_session, test_recipes, test_ingredients):
    ...
    items = [
        models.ShoppingListItem(shopping_list_id=1, ingredient_id=1, quantity=1.0, unit="ч.л", category="Spices"),
        models.ShoppingListItem(shopping_list_id=1, ingredient_id=2, quantity=2.0, unit="ч.л", category="Spices"),
    ]
    for item in items:
        db_session.add(item)
...
class TestIngredientMergeHappyPath:
    def test_merge_ingredients_success(self, client, db_session, test_ingredients, test_recipes, test_shopping_lists):
        response = client.post(
            "/ingredients/merge",
            params={"keep_ingredient_id": 1},
            json=[2]
        )
        ...
        remaining_ingredient = db_session.query(models.Ingredient).filter(models.Ingredient.id == 1).first()
        assert remaining_ingredient is not None
        assert remaining_ingredient.name == "сіль"

        deleted_ingredient = db_session.query(models.Ingredient).filter(models.Ingredient.id == 2).first()
        assert deleted_ingredient is None

    def test_recipe_ingredients_updated_after_merge(self, client, db_session, test_ingredients, test_recipes):
        response = client.post(
            "/ingredients/merge",
            params={"keep_ingredient_id": 1},
            json=[2]
        ...
        recipe_1_ings = db_session.query(models.RecipeIngredient).filter(
            models.RecipeIngredient.recipe_id == 1
        ).all()
        assert len(recipe_1_ings) == 3

        recipe_2_ings = db_session.query(models.RecipeIngredient).filter(
            models.RecipeIngredient.recipe_id == 2
        ).all()
    ...
    def test_shopping_list_items_updated_after_merge(self, client, db_session, test_ingredients, test_recipes, test_shopping_lists):
        response = client.post(
            "/ingredients/merge",
            params={"keep_ingredient_id": 1},
            json=[2]
...
class TestIngredientMergeNoSideEffects:
    def test_unaffected_recipes_unchanged(self, client, db_session, test_ingredients, test_recipes):
        original_recipe_2_count = db_session.query(models.RecipeIngredient).filter(
            models.RecipeIngredient.recipe_id == 2
        ).count()
        ...
        current_recipe_2_count = db_session.query(models.RecipeIngredient).filter(
            models.RecipeIngredient.recipe_id == 2
        ).count()
    ...
    def test_do_merge_function_recipes_updated(self, db_session, test_ingredients, test_recipes):
        db_session.query(models.RecipeIngredient).filter(
            ...
            models.RecipeIngredient.ingredient_id: 1
...
class TestIngredientMergeDataIntegrity:
    def test_recipe_ingredient_relationships_preserved(self, client, db_session, test_ingredients, test_recipes):
        original_ri = db_session.query(models.RecipeIngredient).filter(
            models.RecipeIngredient.recipe_id == 2,
            models.RecipeIngredient.ingredient_id == 2
        ).first()
        ...
        updated_ri = db_session.query(models.RecipeIngredient).filter(
            models.RecipeIngredient.recipe_id == 2,
            models.RecipeIngredient.ingredient_id == 1
        ).first()
    ...
    def test_merged_ingredient_fully_removed(self, client, db_session, test_ingredients, test_recipes):
        response = client.post(
            "/ingredients/merge",
            params={"keep_ingredient_id": 1},
            json=[2]
        ...
        deleted_ingredient = db_session.query(models.Ingredient).filter(
            models.Ingredient.id == 2
        ).first()
        assert deleted_ingredient is None

        deleted_recipe_ing = db_session.query(models.RecipeIngredient).filter(
            models.RecipeIngredient.ingredient_id == 2
        ).first()
        assert deleted_recipe_ing is None

    def test_multiple_merges_affect_correct_ingredients(self, client, db_session, test_ingredients, test_recipes, test_shopping_lists):
