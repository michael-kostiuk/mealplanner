...
from app.services.recipe_import.schemas import RecipeImportDraft
...
def sample_verification_response():
    ...
    return {
        "candidates": [{"content": {"parts": [{"text": '{"results":[{"index":0,"same":true}]}'}]}}]
    }


@pytest.mark.asyncio
async def test_import_recipe_from_image_happy_path(
    db_session,
    mock_ingredient,
    sample_image_bytes,
    sample_valid_ai_response,
    ...
    mock_google_client.generate_content = AsyncMock(return_value=sample_verification_response)
    ...
    with (
        patch(
            "app.services.recipe_import.extractors.image.ImageExtractor.extract",
            side_effect=mock_extract,
        ),
        patch(
            "app.services.recipe_import.pipeline.get_google_ai_client",
            return_value=mock_google_client,
        ),
        patch(
            "app.services.recipe_import.pipeline.get_nutrition_estimator",
            return_value=mock_estimator,
        ),
    ):
        result = await recipe_import_service.start_job(
            "image", sample_image_bytes, mime_type="image/png"
        )
        ...
        assert job.result is not None or (
            job.result.ingredients and len(job.result.ingredients) > 0
        )


@pytest.mark.asyncio
async def test_import_recipe_missing_required_fields(
    db_session, mock_ingredient, sample_image_bytes
):
    """
    ...
    incomplete_response = {
        "candidates": [
            {"content": {"parts": [{"text": '{"name": "Тестова страва", "ingredients": []}'}]}}
        ]
    }
    ...
    with (
        patch(
            "app.services.recipe_import.extractors.image.get_google_ai_client",
            return_value=mock_google_client,
        ),
        patch(
            "app.services.recipe_import.pipeline.get_google_ai_client",
...
@pytest.mark.asyncio
async def test_import_recipe_with_duplicate_ingredients(
    db_session, mock_ingredient, sample_image_bytes
):
    """
...
@pytest.mark.asyncio
async def test_import_recipe_nutrition_estimation_failure(
    db_session, mock_ingredient, sample_image_bytes
):
    """
...
@pytest.mark.asyncio
async def test_import_progress_updates(
    db_session, mock_ingredient, sample_image_bytes, sample_valid_ai_response
):
    """
    Test that job progress is updated correctly through stages.
    """
    mock_extractor_client = MagicMock()
    mock_extractor_client.generate_content = AsyncMock(return_value=sample_valid_ai_response)
    mock_verify_client = MagicMock()
    mock_verify_client.generate_content = AsyncMock(
        return_value={"candidates": [{"content": {"parts": [{"text": '{"results":[]}'}]}}]}
    )
    ...
    with (
        patch(
            "app.services.recipe_import.extractors.image.get_google_ai_client",
            return_value=mock_extractor_client,
        ),
        patch(
            "app.services.recipe_import.pipeline.get_google_ai_client",
            return_value=mock_verify_client,
