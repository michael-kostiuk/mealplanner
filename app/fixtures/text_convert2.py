...
def parse_recipe(text: str) -> dict[str, Any]:
    ...
    for i, line in enumerate(lines[1:], 1):  # Skip first line (name)
        ...
        if (
            line.upper() in ["ІНГРЕДІЄНТИ:", "СПОСІБ ПРИГОТУВАННЯ:", "ККАЛ"]
            or line.endswith("ККАЛ")
            or line.upper().startswith("ІНГРЕДІЄНТИ")
            or line.upper().startswith("СПОСІБ")
        ):
...
def parse_single_ingredient(line: str) -> dict[str, Any] | None:
...
def load_fixtures():
    ...
    for dpart in dparts:
        ...
        try:
            ...
            prompt = (
                f"""based on name of the dish, return weight how good it would fit for meal type. max 1 least 0. and 1-3 tags unrelated to nations of origin.
dish name: {name}
"""
                + """
output format - json:
